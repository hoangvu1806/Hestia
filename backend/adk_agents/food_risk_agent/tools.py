...
def lookup_chemical_identity(query: str) -> dict[str, Any]:
    ...
    try:
        ...
        return {"status": "not_found", "query": query, "detail": str(exc)[:240]}
    cid = item.get("CID")
    return {
        "status": "ok",
        "query": query,
...
def search_reaction_literature(query: str, max_results: int = 5) -> dict[str, Any]:
    ...
    try:
        ...
        return {"status": "error", "query": query, "detail": str(exc)[:240]}
...
def get_pubchem_hazard_summary(query: str) -> dict[str, Any]:
    ...
    try:
        ...
        return {"status": "error", "query": query, "cid": cid, "detail": str(exc)[:240]}
