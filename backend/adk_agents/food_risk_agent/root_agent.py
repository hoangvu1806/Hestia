...
root_agent = LlmAgent(
    ...
    tools=[AgentTool(food_analysis_agent), AgentTool(research_agent)],
...
SPECIALIST ROUTING
Mandatory: call food_analysis_agent when either condition is true:
...
Call research_agent only when the user explicitly asks to find papers, studies, citations, or
...
specialist call per turn and never retry a failed call.
